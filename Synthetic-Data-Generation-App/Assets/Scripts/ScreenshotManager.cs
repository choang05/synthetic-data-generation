...
public class ScreenshotManager : MonoBehaviour
{
    ...
    private string GetFilepath(string dirPath)
    {
        ...
        string mask = string.Format("*_*.{0}", format.ToString().ToLower());
        counter = Directory.GetFiles(folder, mask, SearchOption.TopDirectoryOnly).Length;

        // use width, height, and counter for unique file name
        var filename = string.Format("{0}/{1}_{2}.{3}", folder, CaptureManager.instance.currentCaptureObjName, counter, format.ToString().ToLower());
